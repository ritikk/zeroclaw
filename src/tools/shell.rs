        ...
        match result {
            Ok(Ok(output)) => {
                ...
                if stdout.len() > MAX_OUTPUT_BYTES {
                    let mut bound = MAX_OUTPUT_BYTES;
                    while bound > 0 && !stdout.is_char_boundary(bound) { bound -= 1; }
                    stdout.truncate(bound);
                    stdout.push_str("\n... [output truncated at 1MB]");
                }
                if stderr.len() > MAX_OUTPUT_BYTES {
                    let mut bound = MAX_OUTPUT_BYTES;
                    while bound > 0 && !stderr.is_char_boundary(bound) { bound -= 1; }
                    stderr.truncate(bound);
